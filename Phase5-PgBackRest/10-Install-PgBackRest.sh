...
[pg_cluster_hq]
pg1-host=10.0.0.4  # IP HQ Node 1
pg1-path=/var/lib/postgresql/18/main

pg2-host=10.0.0.5  # IP HQ Node 2
pg2-path=/var/lib/postgresql/18/main

pg3-host=10.0.0.6  # IP HQ Node 3
...
sudo tee /etc/pgbackrest/pgbackrest.conf <<EOF
[global]
repo1-host=10.0.0.30  # IP HQ Backup Server
...
sudo -u postgres pgbackrest --stanza=pg_cluster_hq stanza-create
...
sudo -u postgres pgbackrest --stanza=pg_cluster_hq --type=full backup
